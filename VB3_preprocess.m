...
xlen = length(X);
trjstarts=ones(1,xlen+1);
for k=1:xlen
	trjstarts(k+1) = trjstarts(k) + size(X{k}(:,1:dim),1)-1;
end

% We do not need separate calculations for this array
dat.end = [trjstarts(2:xlen+1)]-1;

% This code can now be parallellized (parfor)
for k=1:xlen 
	dat.dx2(trjstarts(k):(trjstarts(k+1)-1)) = sum(diff(X{k}(:,1:dim),1,1).^2,2);
end
