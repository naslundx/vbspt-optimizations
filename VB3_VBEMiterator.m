...
while(runMore)
    ...
    psiwa2 = psi(W.M.wa(:,2));
    for i =1:N
        lnQ (i,:) = psiwa1(i) + psiwB(i,:) - psiwB0(i) - psiwa0(i);
        lnQ (i,i) = psiwa2(i) - psiwa0(i);
    end
...
%     H = exp(tmp);
    ...
    if(W.N>1) % a is only defined if N>1
        ...
        KL_a=gammaln(wa0)-gammaln(ua0)...
            ...
            -(W.M.wa(:,1)-W.PM.wa(:,1)).*psiwa1...
            +gammaln(W.M.wa(:,2))-gammaln(W.PM.wa(:,2))...
            -(W.M.wa(:,2)-W.PM.wa(:,2)).*psiwa2);
    
        % Jump probablities
        for k=1:W.N
            %ind=setdiff(1:N,k); % only include non-diagonal elements
            ind=(W.PM.wB(k,:)>0); % only include non-zero elements
